...
function CompanyCard({ experience }: { experience: Experience }) {
  ...
  return (
    <article className="warm-card overflow-hidden p-5 sm:p-7">
      <header className="flex flex-col gap-3 border-b border-amber/10 pb-6 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <div className="flex flex-wrap items-center gap-3">
            ...
            {'url' in experience && experience.url && <a href={experience.url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 rounded-full border border-amber/20 px-2.5 py-1 text-[10px] uppercase tracking-wider text-amber">techbazaar.pk <ExternalLink size={11} /></a>}
          </div>
        ...
        {experience.projects.map((project: Project, index: number) => {
          ...
          return (
            <div key={project.name} className="border-b border-amber/[0.08] last:border-0">
              <button type="button" className="flex w-full items-center gap-3 py-5 text-left" onClick={() => setOpenIndex(open ? null : index)} aria-expanded={open}>
  ...
  return (
    <section id="work" className="section-space">
      <div className="page-shell">
        <Reveal><h2 className="section-label">Work</h2></Reveal>
