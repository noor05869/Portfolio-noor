...
export default function Hero() {
  ...
  const container: Variants = {
    hidden: {},
    visible: { transition: { delayChildren: reduceMotion ? 0 : 0.5, staggerChildren: reduceMotion ? 0 : 0.1 } },
  };
  const item: Variants = {
    ...
    <section id="top" className="relative flex min-h-screen items-center overflow-hidden pb-16 pt-28">
      ...
      <motion.div className="page-shell relative z-10 grid items-center gap-14 lg:grid-cols-[1.08fr_.92fr]" variants={container} initial="hidden" animate="visible">
        <div>
          ...
          <motion.h1 variants={item} className="font-display text-[clamp(3rem,10vw,6rem)] font-bold uppercase leading-[0.86] tracking-[-0.065em] text-cream">
            Frontend<br /><span className="text-sand">Engineer</span><span className="text-amber">.</span>
          </motion.h1>
          ...
          <motion.div variants={item} className="mt-9 flex flex-wrap items-center gap-6">
            <a href="#work" className="amber-button">View Work <ArrowDown size={16} aria-hidden="true" /></a>
            <a href="#contact" className="group inline-flex min-h-11 items-center gap-2 text-sm font-medium text-amber">Say Hello <ArrowRight size={16} className="transition-transform group-hover:translate-x-1" aria-hidden="true" /></a>
          </motion.div>
        </div>
        <motion.div variants={item}><LightMoodSwitcher /></motion.div>
      </motion.div>
    </section>
  );
}
