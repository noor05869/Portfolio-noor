    ...
    >
      <nav className="page-shell flex h-16 items-center justify-between" aria-label="Primary navigation">
        <Magnetic strength={0.3}>
          <a href="#top" className="font-display text-lg font-bold tracking-tight text-amber" aria-label="Noor Ullah, back to top" data-cursor="NOOR">NU</a>
        </Magnetic>
        <div className="hidden items-center gap-8 md:flex">
          {navLinks.map((link) => (
            <Magnetic key={link.href} strength={0.2}>
              <a href={link.href} className="group relative py-2 text-sm text-sand transition hover:text-amber" data-cursor="GOTO">
                {link.label}
          ...
          <AudioToggle />
          <Magnetic strength={0.3}>
            <a href={personalInfo.cv} download className="amber-button !min-h-9 !px-4 !py-2" data-cursor="RESUME">
      ...
      {menuOpen && (
        <motion.div id="mobile-menu" initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="border-t border-amber/10 bg-background px-5 pb-5 md:hidden">
          ...
          <div className="mt-4 flex items-center justify-between">
            <span className="text-xs text-sand">Audio Effects</span>
            <AudioToggle />
          </div>
